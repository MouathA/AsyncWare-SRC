...
public class ItemSign extends Item
{
    ...
    public boolean onItemUse(final ItemStack itemStack, final EntityPlayer entityPlayer, final World world, BlockPos offset, final EnumFacing enumFacing, final float n, final float n2, final float n3) {
        ...
        if (enumFacing == EnumFacing.UP) {
            world.setBlockState(offset, Blocks.standing_sign.getDefaultState().withProperty(BlockStandingSign.ROTATION, MathHelper.floor_double((entityPlayer.rotationYaw + 180.0f) * 16.0f / 360.0f + 0.5) & 0xF), 3);
        }
        else {
            world.setBlockState(offset, Blocks.wall_sign.getDefaultState().withProperty(BlockWallSign.FACING, enumFacing), 3);
        }
        --itemStack.stackSize;
