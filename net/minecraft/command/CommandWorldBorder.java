...
public class CommandWorldBorder extends CommandBase
{
    ...
    public void processCommand(final ICommandSender commandSender, final String[] array) throws CommandException {
        ...
        if (array[0].equals("set")) {
            ...
            final double double2 = CommandBase.parseDouble(position.getX() + 0.5, array[1], true);
            final double double3 = CommandBase.parseDouble(position.getZ() + 0.5, array[2], true);
            worldBorder.setCenter(double2, double3);
            CommandBase.notifyOperators(commandSender, this, "commands.worldborder.center.success", double2, double3);
        }
            ...
            if (array[1].equals("buffer")) {
                ...
                final double double4 = CommandBase.parseDouble(array[2], 0.0);
                final double damageBuffer = worldBorder.getDamageBuffer();
                worldBorder.setDamageBuffer(double4);
                CommandBase.notifyOperators(commandSender, this, "commands.worldborder.damage.buffer.success", String.format("%.1f", double4), String.format("%.1f", damageBuffer));
            }
            else if (array[1].equals("amount")) {
                if (array.length != 3) {
                    throw new WrongUsageException("commands.worldborder.damage.amount.usage", new Object[0]);
                }
                final double double5 = CommandBase.parseDouble(array[2], 0.0);
                final double damageAmount = worldBorder.getDamageAmount();
                worldBorder.setDamageAmount(double5);
                CommandBase.notifyOperators(commandSender, this, "commands.worldborder.damage.amount.success", String.format("%.2f", double5), String.format("%.2f", damageAmount));
            }
        }
    ...
    public List addTabCompletionOptions(final ICommandSender commandSender, final String[] array, final BlockPos blockPos) {
        return (array.length == 1) ? CommandBase.getListOfStringsMatchingLastWord(array, "set", "center", "damage", "warning", "add", "get") : ((array.length == 2 && array[0].equals("damage")) ? CommandBase.getListOfStringsMatchingLastWord(array, "buffer", "amount") : ((array.length >= 2 && array.length <= 3 && array[0].equals("center")) ? CommandBase.func_181043_b(array, 1, blockPos) : ((array.length == 2 && array[0].equals("warning")) ? CommandBase.getListOfStringsMatchingLastWord(array, "time", "distance") : null)));
    }
}
