...
public class MessageDeserializer extends ByteToMessageDecoder
{
    ...
    protected void decode(final ChannelHandlerContext channelHandlerContext, final ByteBuf byteBuf, final List list) throws InstantiationException, IllegalAccessException, Exception, IOException {
        if (byteBuf.readableBytes() != 0) {
            ...
            if (MessageDeserializer.logger.isDebugEnabled()) {
                MessageDeserializer.logger.debug(MessageDeserializer.RECEIVED_PACKET_MARKER, " IN: [{}:{}] {}", new Object[] { channelHandlerContext.channel().attr(NetworkManager.attrKeyConnectionState).get(), varIntFromBuffer, packet.getClass().getName() });
            }
        }
    }
}
