...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    public void renderItemOverlayIntoGUI(final FontRenderer fontRenderer, final ItemStack itemStack, final int n, final int n2, final String s) {
        if (itemStack != null) {
            if (itemStack.stackSize != 1 || s != null) {
                String string = (s == null) ? String.valueOf(itemStack.stackSize) : s;
                if (s == null && itemStack.stackSize < 1) {
                    string = EnumChatFormatting.RED + String.valueOf(itemStack.stackSize);
                }
                fontRenderer.drawStringWithShadow(string, (float)(n + 19 - 2 - fontRenderer.getStringWidth(string)), (float)(n2 + 6 + 3), 16777215);
            }
            if (itemStack.isItemDamaged()) {
                ...
                this.func_181565_a(worldRenderer, n + 2, n2 + 13, 13, 2, 0, 0, 0, 255);
                this.func_181565_a(worldRenderer, n + 2, n2 + 13, 12, 1, (255 - n4) / 4, 64, 0, 255);
                this.func_181565_a(worldRenderer, n + 2, n2 + 13, n3, 1, 255 - n4, n4, 0, 255);
            }
        }
    }
    ...
    private void renderQuads(final WorldRenderer worldRenderer, final List list, final int n, final ItemStack itemStack) {
        final boolean b = n == -1 && itemStack != null;
        while (0 < list.size()) {
            final BakedQuad bakedQuad = list.get(0);
            int n2 = n;
            if (b && bakedQuad.hasTintIndex()) {
                int n3 = itemStack.getItem().getColorFromItemStack(itemStack, bakedQuad.getTintIndex());
                if (EntityRenderer.anaglyphEnable) {
                    n3 = TextureUtil.anaglyphColor(n3);
                }
                n2 = (n3 | 0xFF000000);
            }
            this.renderQuad(worldRenderer, bakedQuad, n2);
            int n4 = 0;
            ++n4;
        }
    }
    ...
    protected void renderItemModelTransform(final ItemStack itemStack, final IBakedModel bakedModel, final ItemCameraTransforms.TransformType transformType) {
        ...
        if (itemCameraTransforms.getTransform(transformType) < 0) {
    ...
    private void renderModel(final IBakedModel bakedModel, final int n, final ItemStack itemStack) {
        ...
        while (0 < values.length) {
            this.renderQuads(worldRenderer, bakedModel.getFaceQuads(values[0]), n, itemStack);
            int n2 = 0;
            ++n2;
        }
    ...
    private void func_181565_a(final WorldRenderer worldRenderer, final int n, final int n2, final int n3, final int n4, final int n5, final int n6, final int n7, final int n8) {
        ...
        worldRenderer.pos(n + 0, n2 + 0, 0.0).color(n5, n6, n7, n8).endVertex();
        worldRenderer.pos(n + 0, n2 + n4, 0.0).color(n5, n6, n7, n8).endVertex();
        worldRenderer.pos(n + n3, n2 + n4, 0.0).color(n5, n6, n7, n8).endVertex();
        worldRenderer.pos(n + n3, n2 + 0, 0.0).color(n5, n6, n7, n8).endVertex();
